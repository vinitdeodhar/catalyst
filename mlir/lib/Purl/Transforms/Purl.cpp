...
// The pass
//===----------------------------------------------------------------------===//
struct PurlPass : impl::PurlPassBase<PurlPass> {
    using PurlPassBase::PurlPassBase;
    ...
    void processLoop(scf::WhileOp loop)
    {
        ...
        StringRef klassStr = info.klass == Klass::Carry     ? "carry"
                             : info.klass == Klass::Restart ? "restart"
                                                            : "unknown";
        loop->setAttr("purl.class", b.getStringAttr(klassStr));

        if (info.klass == Klass::Unknown) {
            loop.emitRemark("purl: unknown class; pass does not fire");
            return;
        }
        ...
        loop->setAttr("purl.carry_slot", b.getI64IntegerAttr(info.carrySlot));

        bool condMeas = conditionFromMeasure(loop);
        BodyStats bs;
        double B = bodyDepth(loop, c, condMeas, &bs);
        if (c.unit)
            loop->setAttr("purl.body_layers", b.getF64FloatAttr(B));
        else
            loop->setAttr("purl.body_seconds", b.getF64FloatAttr(B));
                    ...
                    Pauli axis, Pauli corr, const PrepChain *prep)
    {
        ...
        Block *blk = new Block();
        reg->push_back(blk);
        Value fresh = blk->addArgument(qT, loc);
        OpBuilder rb(ctx);
        rb.setInsertionPointToStart(blk);
        Value prepared = fresh;
        if (prep)
                                ...
                                int slot = -1)
    {
        ...
        nl->setAttr("purl.C", b.getI64IntegerAttr(C));
        nl->setAttr("purl.window", b.getDenseI64ArrayAttr({win.cMin, win.cMax}));
        nl->setAttr("purl.cut", b.getStringAttr("deterministic"));
