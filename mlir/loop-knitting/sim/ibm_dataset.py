...
def carried_calib(qubit=0, path=JSON_PATH, p_leak=IBM_LEAK_PER_2Q):
    ...
    return {
        ...
        "p_leak": p_leak, "p_leak_ro": p_leak * 0.5, "p_prep": d["p_prep"],
        # leakage as an idle time-constant for the trajectory sim (from p_leak
        # per 2q gate, spread over the body duration); set by the eval.
        "T_leak": float("inf"),
    }
