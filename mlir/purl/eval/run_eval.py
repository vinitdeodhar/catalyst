...
# Evaluation calibration: weak T1/T2, low gate/readout error, and per-2q-gate
# leakage (spec 4.1) -- reset-clearable, so it only bites on a target-entangling
# benchmark (rus_lowp's touch); rus_rx_ibm holds the target idle -> no leakage.
EVAL_CALIB = {
...
def variance_V(C, p, gamma2=16.0):
    """KNIT sampling-variance factor V(C) = (1-q)/(1-gamma2*q), q=(1-p)^C
    (spec 3.5). Returns +inf where gamma2*q >= 1 (variance diverges)."""
    q = (1.0 - p) ** C
    denom = 1.0 - gamma2 * q
    return float("inf") if denom <= 0.0 else (1.0 - q) / denom


def window(p, gamma2=16.0, f=0.05, vmax=4.0, calib=None):
    """Pass cut-period window (spec 3.3). Returns (C_min, C_max).

    C_min = smallest C with the KNIT variance capped, V(C) <= vmax (default 4) --
    a variance cap, not bare finiteness (the vmax->inf limit recovers the old
    ceil(ln(gamma2)/ln(1/(1-p)))). C_max = floor(f*T2/(B+tau)) coherence budget.
    An empty window (C_min > C_max) means KNIT is inadmissible here."""
    C_min = 1
    while variance_V(C_min, p, gamma2) > vmax:
        C_min += 1
        if C_min > 100000:            # pathological p; give up gracefully
            break
