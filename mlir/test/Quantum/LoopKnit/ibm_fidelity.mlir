// RUN: quantum-opt --loop-knit="calib=%S/ibm_eagle_r3.json p=0.1 shots=200000 p-leak=1e-3 carry-qubit=0 depth=10 analyze-only=true" %s | FileCheck %s
//
...
// CHECK: knit.body_seconds
// per-depth prediction requested via depth=10
// CHECK: knit.fidelity_at_depth
// mean delivered fidelity at the bounded vs unbounded depth
// CHECK: knit.predicted_fidelity = {bounded = {{[0-9.eE+-]+}} : f64, unbounded = {{[0-9.eE+-]+}} : f64}
