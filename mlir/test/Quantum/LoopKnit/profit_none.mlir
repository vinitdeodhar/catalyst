// RUN: quantum-opt --loop-knit="calib=%S/backend_clean.json p=0.625 shots=20000" --verify-diagnostics %s | FileCheck %s
//
// Profitability model (3.5) under a CLEAN calibration: no strategy beats leaving
// the loop alone by the margin, so `knit.strategy = "none"` and the loop is left
...
func.func @held_memory() -> f64 {
  ...
  // expected-remark@+1 {{loop-knit: not profitable; loop left unchanged}}
