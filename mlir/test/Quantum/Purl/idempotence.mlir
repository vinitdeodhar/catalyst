// RUN: quantum-opt --purl="calib=unit p=0.625 C=4" --purl="calib=unit p=0.625 C=4" --purl-lower-qcut %s | FileCheck %s
//
// Running the pass twice equals running it once: the purl.applied guard blocks a
// second carry extension, so the carry stays 4-element (not 6) and there is a
// single purl_sample_term declaration.
